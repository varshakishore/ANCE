...
def write_qas_query(args, qas_file, out_query_file):
    ...
    with open(qas_path, "r", encoding="utf-8") as f, open(out_query_path, "wb") as out_query:
        ...
        for row in reader:
            row = json.loads(row)
            question = normalize_question(row['doc_text'])
...
def write_query_rel(args, pid2offset, query_file, out_query_file, out_ann_file, out_train_file, passage_id_name="passage_id"):
    ...
    # TODO remove absolute path here
    doc2ques = pkl.load(open('/home/zl327/NQ320K_ANCE/docid2quesid.pkl', 'rb'))
    ...
    with open(out_query_path, "wb") as out_query, \
        ...
        for sample in data:
            positive_ctxs = sample['doc_id']
            possible_positives = doc2ques[int(positive_ctxs)]
            neg_ctxs = sample['neg_sample']
            question = normalize_question(sample['doc_text'])
            # filter of questions that are not in the generated passages
            possible_positives = [item for item in possible_positives if item<3151956]
            first_pos_pid = pid2offset[int(np.random.choice(possible_positives))]
            pos_pids = [str(pid2offset[int(pos_ctx)]) for pos_ctx in possible_positives]
            neg_pids = [str(pid2offset[int(neg_ctx['ques_id'])]) for neg_ctx in neg_ctxs]
            out_ann.write("{}\t{}\t{}\n".format(qid, ','.join(pos_pids), sample["doc_id"]))
...
def preprocess(args):
    ...
    if args.data_type == 0:
        write_query_rel(args, pid2offset, "trainqueries.json", "train-query", "train-ann", "train-data")
    ...
    if args.data_type != 1:
        write_query_rel(args, pid2offset, "testqueries.json", "dev-query", "dev-ann", "dev-data")
        write_qas_query(args, "testqueries.json", "test-query")
    if args.data_type != 0:
...
def PassagePreprocessingFn(args, line, tokenizer):
    ...
    p_id = int(line_dict['ques_id'])
    text = line_dict['doc_text']
